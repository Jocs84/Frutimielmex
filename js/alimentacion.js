// *********************************************************************************
// ***
// *** ALIMENTACION.JS se encarga de manipular el DOM y efectos del archivo
                ...
                // Si se recibe un objeto JSON con el resultado de la búsqueda
                   ...
                   //  Vacia el contenedor en caso de busquedas anteriores
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        rem = rem.replace("%FECHA%",pad(data[i]["DiaCadIng"],2) + "/" + pad(data[i]["MesCadIng"],2) + "/" + pad(data[i]["AnioCadIng"],4));
                ...
                if(data.estado == '2'){
                   ...
                   //  Vacia el contenedor en caso de busquedas anteriores
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        // Poner la fecha de caducidad del alimento
                        rem = rem.replace("%FECHA%",pad(data[i]["DiaCadAli"],2) + "/" + pad(data[i]["MesCadAli"],2) + "/" + pad(data[i]["AnioCadAli"],4));
    ...
    $(document).on('click', '.edElemAli', function(e) {
       ...
       for (var i = 0; i < jsonBusq.length; i++) {
           ...
           if(jsonBusq[i]["IdAlimento"] === nom ){
               ...
               var fecha = pad(jsonBusq[i]["AnioCadAli"],4) + "-" + pad(jsonBusq[i]["MesCadAli"],2) + "-" + pad(jsonBusq[i]["DiaCadAli"],2);
                ...
                if(data.estado == '2'){
                   ...
                   //  Vacia el contenedor en caso de busquedas anteriores
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        rem = rem.replace("%FECHA%",pad(data[i]["DiaPrep"],2) + "/" + pad(data[i]["MesPrep"],2) + "/" + pad(data[i]["AnioPrep"],4));
        ...
        for (var i = 0; i < jsonBusq.length; i++) {
            ...
            if(jsonBusq[i]["IdPreparacion"] === id && jsonBusq[i]["NombreAlimento"] === nom){
                ...
                if (navigator.userAgent.search("Firefox") >= 0) {
                    fecha1 = pad (jsonBusq[i]["DiaPrep"], 2) + "-" + pad (jsonBusq[i]["MesPrep"], 2) + "-" + pad(jsonBusq[i]["AnioPrep"],4);
                    fecha2 = pad (jsonBusq[i]["DiaCadPrep"] , 2)+ "-" + pad (jsonBusq[i]["MesCadPrep"], 2) + "-" + pad(jsonBusq[i]["AnioCadPrep"],4);
                } else {
                    fecha1 = pad(jsonBusq[i]["AnioPrep"],4) + "-" + pad (jsonBusq[i]["MesPrep"], 2) + "-" + pad (jsonBusq[i]["DiaPrep"], 2);
                    fecha2 = pad(jsonBusq[i]["AnioCadPrep"],4) + "-" + pad (jsonBusq[i]["MesCadPrep"], 2) + "-" + pad (jsonBusq[i]["DiaCadPrep"] , 2);
                }
